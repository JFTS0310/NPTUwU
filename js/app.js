...
periodOrder.forEach(item => {
    ...
    timeHeader.style.minWidth = "150px"; 
    timeHeader.style.width = "150px";
    
    timeHeader.style.padding = "10px 15px"; 
    timeHeader.style.verticalAlign = "middle";

    const codeDiv = document.createElement("div");
    codeDiv.textContent = period;
    codeDiv.style.fontSize = "1.5em"; 
    codeDiv.style.fontWeight = "bold";
    codeDiv.style.lineHeight = "1";
...
Promise.all([
    ...
    .then(response => {
        const [data, departmentRaw] = response;
        ...
        function flattenDepartments(data) {
            let result = {};
            for (const key in data) {
                if (typeof data[key] === 'object' && data[key] !== null) {
                    Object.assign(result, flattenDepartments(data[key]));
                } else {
                    result[key] = data[key];
                }
            }
        ...
        const allDepartments = flattenDepartments(departmentRaw);
